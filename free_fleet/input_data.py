...
def _iter_json_records(source: Path) -> Iterator[object]:
    ...
    with open(source, mode="r", encoding="utf-8-sig", errors="replace") as handle:
...
def _select_item(item: InputItem, target_ids: set[str] | None) -> InputItem | None:
...
def _validate_input_item(
    ...
    return _select_item(item, target_ids)
    ...
    if suffix == ".jsonl":
        with open(source, mode="r", encoding="utf-8-sig", errors="replace") as handle:
            for line_number, line in enumerate(handle, start=1):
                ...
                item = _validate_input_item(raw_item, parsed_count, seen, target_ids)
                parsed_count += 1
                if item is not None:
                    yield item
    elif suffix == ".json":
        ...
        try:
            with open(source, mode="r", encoding="utf-8-sig", errors="replace") as f:
                ...
                for row_idx, row in enumerate(reader, start=1):
                    ...
                    item = _validate_input_item(raw_item, row_idx - 1, seen, target_ids)
                    parsed_count += 1
                    if item is not None:
                        yield item
        ...
        item = _validate_input_item(raw_item, 0, seen, target_ids)
        parsed_count = 1
        if item is not None:
            yield item
