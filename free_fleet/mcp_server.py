...
def create_mcp_server(workspace_root: str | Path, db_path: str | Path | None = None) -> FastMCP:
    ...
    @server.tool(structured_output=True)
    def free_fleet_tasks() -> TasksResult:
    ...
    @server.tool(structured_output=True)
    def free_fleet_doctor() -> DoctorReport:
