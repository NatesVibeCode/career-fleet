...
def process(
    ...
    **load_kwargs,
    ...
    if isinstance(input, list):
        ...
        raw_items_factory = lambda: iter_input_items(source, **input_options)
        ...
        try:
            if hasattr(input, "to_dict") and hasattr(input, "columns"):
                ...
                def dataframe_items():
                    columns = list(input.columns)  # type: ignore
                    if hasattr(input, "itertuples"):
                        rows = (
                            dict(zip(columns, values))
                            for values in input.itertuples(index=False, name=None)  # type: ignore
                        )
                    else:
                        rows = iter(input.to_dict(orient="records"))  # type: ignore
