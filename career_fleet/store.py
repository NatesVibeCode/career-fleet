...
CREATE TABLE IF NOT EXISTS evaluations (
    ...
    status TEXT NOT NULL,           -- qualified, disqualified, marginal, error
...
class CareerStore:
    ...
    def get_company_dossier(self, company_id: str) -> Optional[Dict[str, Any]]:
        with self.connect() as con:
            ...
            if not comp:
                return None
            jobs = con.execute("SELECT id, title, location, is_remote, job_url FROM job_postings WHERE company_id = ?", (company_id,)).fetchall()
